...
async function main() {
  ...
  ensureDir(logsDir);
  const ts = timestamp();
  const logPathTs = path.join(logsDir, `test-${ts}.log`);
  const logPathLatest = path.join(logsDir, 'test-latest.log');
  ...
  const child = spawn(process.execPath, [jestBin, '--runInBand'], { stdio: ['inherit', 'pipe', 'pipe'] });
